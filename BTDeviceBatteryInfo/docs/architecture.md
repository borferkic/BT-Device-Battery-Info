...
- `TaskbarWidgetController.cs`: presenta pastillas de los dispositivos conectados compatibles usando la colección ya publicada por `MainViewModel`; no inicia otro watcher ni consultas de batería. La clasificación usa las categorías, clase Bluetooth o apariencia LE publicadas por Windows. Su ventana auxiliar se acopla a `Shell_TrayWnd` si UI Automation indica una zona libre a la izquierda y se vuelve a evaluar periódicamente; los cambios de visibilidad y los motivos de ocultamiento se anotan en el log local. Los iconos de dispositivo son geometrías Lucide (`Services/LucideIcons.cs`) en `Elegant Black` y glifos de Windows (Segoe Fluent Icons / Segoe MDL2 Assets) en `System`; la licencia de Lucide se incluye junto al ejecutable.
...
La batería se intenta obtener desde `System.Devices.BatteryLife` y desde la propiedad raw `{104EA319-6EE2-4701-BD47-8DDBF425BBE5} 2` (`DEVPKEY_Bluetooth_BatteryLevel`) en el endpoint, el `DeviceContainer` y todos los nodos PnP del contenedor. La búsqueda PnP enumera los devnodes `BTHLE` y `BTHENUM` registrados y los vincula exclusivamente por `ContainerId` (`8C7ED206-3F8A-4827-B3AB-AE9E1FAEFC6C`); no contiene nombres, rutas ni direcciones de una marca concreta. Esto cubre los nodos Classic HFP/Hands-Free que Windows puede mantener fuera de la lista `present` aunque sigan exponiendo `DEVPKEY_Bluetooth_BatteryLevel`. GATT estándar `180F`/`2A19` permanece como fallback para dispositivos BLE. Los candidatos se deduplican por dirección. Hay como máximo dos consultas GATT activas en toda la aplicación.
...
Las generaciones de conexión invalidan resultados anteriores cuando cambian los endpoints. Si el contenedor ya no tiene ningún endpoint conectado, se descarta inmediatamente su caché y la interfaz recibe batería no disponible; nunca reutiliza un porcentaje para una fila desconectada. Al finalizar una consulta obsoleta se vuelve a evaluar el contenedor actual. Los fallos sin valor previo programan reintentos a los 3, 10 y luego 20 segundos desde la finalización; un valor previo mantiene el intervalo de dos minutos. Los valores de hidratación caducan a los diez minutos. El cierre cancela los reintentos y consultas en cola, y suprime resultados de batería posteriores.

El log local registra la fuente ganadora y el tiempo de cada lectura, por ejemplo `PnP HFP native`, `PnP container`, `PnP device` o `GATT`; no incluye nombre, dirección ni identificador del dispositivo.
